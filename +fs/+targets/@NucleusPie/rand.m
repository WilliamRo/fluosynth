...
if nargin < 2, distort = fs.targets.Nucleus.DefaultDistort; end
if nargin < 1, minor = fs.targets.Nucleus.DefaultMinor; end
num_points = 8;

%% rand an outline
major = minor * fs.utils.truncated_randn(...
    [1, fs.targets.Nucleus.MaxAspectRatio]);
...
%% create nucleus
thickness = fs.targets.Nucleus.DefaultThickness;
nucleus = fs.targets.Nucleus(initpoints, thickness);
